...
8. **Conversational Verdict (shoppingAdvice)**: Write a single, highly-engaging paragraph (2-3 sentences) of shopping advice specifically tailored for this product. Use a conversational, helpful tone. Mention key trade-offs.
9. **Representative Reviews (representativeReviews)**: Extract exactly 3 representative quotes from the customer reviews provided: one positive review quote, one neutral review quote, and one negative review quote. These must reflect real customer statements from the provided reviews. If no reviews are available, return placeholders.
...
Return ONLY a valid JSON object matching the following structure:
{
  ...
  "analysis": {
    ...
    "shoppingAdvice": "If you are upgrading from older headphones, this is a massive jump in comfort and battery. However, if you already own the previous generation, the sound quality improvement is incremental, and you might want to wait for a price drop."
  },
  "representativeReviews": {
    "positive": "Absolutely love the battery life! Lasts me a whole week of daily commutes without a single charge.",
    "neutral": "The sound quality is good and well-balanced, but the plastic build feels a bit cheap for the premium price tag.",
    "negative": "The active noise cancellation is decent but it struggles with high-pitched sounds like train squeals."
  },
  "alternatives": [
    {
...
/**
 * Generates an AI-driven comparison verdict between two audited products.
 * @param {object} p1 - Analysis result of product 1
 * @param {object} p2 - Analysis result of product 2
 * @returns {Promise<string>}
 */
export async function compareProducts(p1, p2) {
  const apiKey = process.env.GROQ_API_KEY;
  if (!apiKey) {
  ...
  const systemPrompt = `You are TruthBuy, an elite shopping consultant.
Your job is to compare two products and write a concise, objective comparison verdict (2-3 sentences max).
Explain which is the better purchase based on their trust scores, ratings, pros, cons, and sentiments. 
If one has a significantly higher fake review risk, prioritize calling that out as a reason to avoid it.
Keep your response conversational, direct, and completely free of markdown formatting.`;

  const userPrompt = `Product 1:
Title: ${p1.productInfo.title}
Trust Score: ${p1.trustScore}%
Rating: ${p1.rating}
Pros: ${p1.analysis.pros.join(', ')}
Cons: ${p1.analysis.cons.join(', ')}

Product 2:
Title: ${p2.productInfo.title}
Trust Score: ${p2.trustScore}%
Rating: ${p2.rating}
Pros: ${p2.analysis.pros.join(', ')}
Cons: ${p2.analysis.cons.join(', ')}`;
  ...
  try {
    ...
    return completion.choices[0]?.message?.content?.trim() || 'No comparison verdict generated.';
  } catch (error) {
    console.error('[Groq Analyzer] Comparison failed:', error);
    return 'Failed to generate comparison verdict.';
  }
}
